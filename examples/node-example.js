 ...
 * Since client-store relies on localStorage which is a browser API,
...
if (db.storageExists === false) {
  ...
  console.log(
    'Created "products" table with columns: name, category, price, inStock'
  );
} else {
  ...
  if (db.tableExists("products")) {
    console.log(
      `Products table exists with ${db.rowCount("products")} records`
    );
  }
}
...
const electronics = db.query("products", {
